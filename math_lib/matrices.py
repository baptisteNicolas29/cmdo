...
def is_same_length(mtx1: list[int | float], mtx2: list[int | float]) -> bool:
    """
...
def is_identity_4x4(mtx1: list[int | float],  epsilon: float = 0.0001) -> bool:
    """
    ...
    return is_almost_same(mtx1, identity_4x4, epsilon=epsilon)
...
def is_almost_same(
    ...
    """
    return (
            is_same_length(mtx1, mtx2) and
            all(abs(x1 - x2) < epsilon for x1, x2 in zip(mtx1, mtx2))
    )


def mtx_multiply(mtx1: list[int | float], mtx2: list[int | float]):
    if not is_same_length(mtx1, mtx2):
